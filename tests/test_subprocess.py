...
def test_run_remote_same_machine(tmp_path):
    ...
    stdout, stderr = subprocess_run(os.environ.get('HOSTNAME', None), ['cd', str(tmp_path), ';', 'ls', 'file1', ';', 'echo', '$USER'], remsh_cmd=local_ssh)
    assert stdout.splitlines() == ['file1', os.environ['USER']]

    stdout, stderr = subprocess_run(os.environ.get('HOSTNAME', None), ['cd', str(tmp_path), ';', 'ls', 'file 2'], remsh_cmd=local_ssh)
    assert stdout.splitlines() == ['file 2']
...
def test_copy_same_machine(tmp_path, change_test_dir):
    ...
    os.chdir(tmp_path)
    subprocess_copy(['test_file_1', 'test_file_2'], to_dir, to_host=os.environ.get('HOSTNAME', None), remsh_cmd=local_ssh)
    ...
    subprocess_copy(['test_file_1', tmp_path / 'from_dir' / 'test_file_3'], to_dir, to_host=os.environ.get('HOSTNAME', None), remsh_cmd=local_ssh)
    with open(to_dir / 'test_file_1') as fin:
        assert fin.readlines() == [ '1\n' ]
    ...
    subprocess_copy('from_dir', to_dir, to_host=os.environ.get('HOSTNAME', None), remsh_cmd=local_ssh)
    ...
    with open(Path.home() / 'pytest_test_file_4', 'w') as fout:
        fout.write('4\n')
    subprocess_copy('pytest_test_file_4', to_dir, from_host=os.environ.get('HOSTNAME', None), remsh_cmd=local_ssh)
